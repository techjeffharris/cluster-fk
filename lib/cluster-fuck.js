...
function ClusterFuck (options) {
	...
	function startRepl() {
		...
		repl.on('starting', function () {
			console.log('starting REPL server...');
		});

		repl.on('ready', function () {
			console.log('REPL server ready!');
		});

		repl.on('stopping', function () {
			console.log("REPL server stopping...");
		});

		repl.on('stopped', function () {
	...
	function stopWorkers (done) {
		...
		if (utils.getType(done) === 'function') {
			...
			throw Error ('"done" is not a function! :(');
		}
